...
import { adc_Direct_Byte, adc_Immediate_Byte, adc_Immediate_Word } from "./adc";
...
describe("ADC #const (byte)", () => {
  test("addition setting no flag", () => {
    const state = StateFromScratch();
    state.flags = Flag.M;
    const ctx = ContextFromState(state);
    const output = { state: { a: 0x0001, flags: Flag.M } };
    expect(adc_Immediate_Byte(0x01, state, ctx)).toStrictEqual(output);
  });
  ...
  test("addition setting carry flag", () => {
    ...
    expect(adc_Immediate_Byte(0x03, state, ctx)).toStrictEqual(output);
  });
  ...
  test("addition setting overflow flag", () => {
    ...
    expect(adc_Immediate_Byte(0x40, state, ctx)).toStrictEqual(output);
  });
  ...
  test("addition setting negative flag", () => {
    ...
    expect(adc_Immediate_Byte(0x80, state, ctx)).toStrictEqual(output);
  });
  ...
  test("addition setting overflow and negative flag", () => {
    ...
    expect(adc_Immediate_Byte(0xc0, state, ctx)).toStrictEqual(output);
  });
  ...
  test("addition not overriding high byte of A", () => {
    ...
    expect(adc_Immediate_Byte(0x02, state, ctx)).toStrictEqual(output);
  });
});

describe("ADC #const (word)", () => {
  test("addition setting no flag", () => {
    const state = StateFromScratch();
    const ctx = ContextFromState(state);
    const output = { state: { a: 0x0001, flags: 0 } };
    expect(adc_Immediate_Word(0x0001, state, ctx)).toStrictEqual(output);
  });
  ...
  test("addition setting carry flag", () => {
    ...
    expect(adc_Immediate_Word(0x0003, state, ctx)).toStrictEqual(output);
  });
  ...
  test("addition setting overflow flag", () => {
    ...
    expect(adc_Immediate_Word(0x4000, state, ctx)).toStrictEqual(output);
  });
  ...
  test("addition setting negative flag", () => {
    ...
    expect(adc_Immediate_Word(0x8000, state, ctx)).toStrictEqual(output);
  });
  ...
  test("addition setting overflow and negative flag", () => {
    ...
    expect(adc_Immediate_Word(0xc000, state, ctx)).toStrictEqual(output);
  });
