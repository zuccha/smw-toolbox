...
describe.each(modes)("$mode", ({ mode, arg }) => {
  describe("A 8-bit", () => {
    test.each`
      ...
      ${0xf0} | ${{ a: 0xfff0 }}            | ${{ a: 0xff00, flag_n: 0, flag_z: 1 }}
      ${0x00} | ${{ a: 0xff0f }}            | ${{ a: 0xff0f, flag_n: 0, flag_z: 0 }}
      ${0x0f} | ${{ a: 0xff0f }}            | ${{ a: 0xff00, flag_n: 0, flag_z: 1 }}
      ${0x01} | ${{ a: 0x0010, flag_n: 1 }} | ${{ a: 0x0011, flag_n: 0 }}
      ${0x01} | ${{ a: 0x0010, flag_z: 1 }} | ${{ a: 0x0011, flag_z: 0 }}
    `(`EOR ${mode} [$initialProcessor]`, (params) => {
      run({ opcode: "EOR", arg, mode, ...params });
    });
  });
  ...
  describe("A 16-bit", () => {
    test.each`
      ...
      ${0xf000} | ${{ a: 0xfff0 }}            | ${{ a: 0x0ff0, flag_n: 0, flag_z: 0 }}
      ${0x0000} | ${{ a: 0xff0f }}            | ${{ a: 0xff0f, flag_n: 1, flag_z: 0 }}
      ${0xf0f0} | ${{ a: 0xf0f0 }}            | ${{ a: 0x0000, flag_n: 0, flag_z: 1 }}
    ...
    `(`EOR ${mode} [$initialProcessor]`, (params) => {
      ...
      run({ opcode: "EOR", arg, mode, ...params, initialProcessor });
    });
  });
});
