...
describe.each(modes)("$mode", ({ mode, arg }) => {
  describe("A 8-bit", () => {
    test.each`
      ...
      ${0x0001} | ${{ flag_c: 1 }} | ${0x0002}     | ${{ flag_c: 0 }}
    `(`ASL ${mode} [$initialProcessor]`, (params) => {
      run({ opcode: "ASL", arg, mode, ...params });
    });
  });
  ...
  describe("A 16-bit", () => {
    test.each`
      ...
      ${0x0100} | ${{ flag_c: 1 }} | ${0x0200}     | ${{ flag_c: 0 }}
    `(`ASL ${mode} [$initialProcessor]`, (params) => {
      const initialProcessor = { ...params.initialProcessor, flag_m: 0 };
      run({ opcode: "ASL", arg, mode, ...params, initialProcessor });
    });
  });
});
