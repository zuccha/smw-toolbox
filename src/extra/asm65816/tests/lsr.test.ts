...
describe("'A'", () => {
  describe("A 8-bit", () => {
    test.each`
      ...
      ${{ a: 0x0004, flag_c: 1 }} | ${{ a: 0x0002, flag_c: 0 }}
    `(`LSR A [$initialProcessor]`, (params) => {
      run({ opcode: "LSR", mode: "A", ...params });
    });
  });
...
describe.each(modes)("$mode", ({ mode, arg }) => {
  describe("A 8-bit", () => {
    test.each`
      ...
      ${0x0004} | ${{ flag_c: 1 }} | ${0x0002}     | ${{ flag_c: 0 }}
    `(`LSR ${mode} [$initialProcessor]`, (params) => {
      run({ opcode: "LSR", arg, mode, ...params });
    });
  });
  ...
  describe("A 16-bit", () => {
    test.each`
      ...
      ${0x0080} | ${{}}            | ${0x0040}     | ${{}}
      ${0x0100} | ${{}}            | ${0x0080}     | ${{}}
      ${0x8000} | ${{ flag_n: 1 }} | ${0x4000}     | ${{ flag_n: 0 }}
      ${0x8111} | ${{ flag_c: 1 }} | ${0x4088}     | ${{ flag_c: 1 }}
      ${0x0001} | ${{}}            | ${0x0000}     | ${{ flag_c: 1, flag_z: 1 }}
      ${0x0400} | ${{ flag_z: 1 }} | ${0x0200}     | ${{ flag_z: 0 }}
      ${0x0400} | ${{ flag_n: 1 }} | ${0x0200}     | ${{ flag_n: 0 }}
      ${0x0400} | ${{ flag_c: 1 }} | ${0x0200}     | ${{ flag_c: 0 }}
    `(`LSR ${mode} [$initialProcessor]`, (params) => {
      const initialProcessor = { ...params.initialProcessor, flag_m: 0 };
      run({ opcode: "LSR", arg, mode, ...params, initialProcessor });
    });
  });
});
